...
def add_args(parser):
    ...
    parser.add_argument('--config_file', required=False,
                        default="configs/physllm_multisource_train_example.yaml", type=str, help="Path to the config file.")
...
def train_and_test(config, data_loader_dict):
    ...
    elif config.MODEL.NAME == 'PhysLLM':
        model_trainer = trainer.PhysLLMTrainer.PhysLLMTrainer(config, data_loader_dict)
    elif config.MODEL.NAME == 'NewPhysLLM':
        model_trainer = trainer.NewPhysLLMTrainer.NewPhysLLMTrainer(config, data_loader_dict)
    elif config.MODEL.NAME == 'RhythmFormer':
        model_trainer = trainer.RhythmFormerTrainer.RhythmFormerTrainer(config, data_loader_dict)
    elif config.MODEL.NAME == 'RhythmFormer':
        model_trainer = trainer.RhythmFormerTrainer.RhythmFormerTrainer(config, data_loader_dict)   
    elif config.MODEL.NAME == 'MDNet':
        model_trainer = trainer.MDNetTrainer.MDNetTrainer(config, data_loader_dict)   
    elif config.MODEL.NAME == 'ContrastPhys':
        model_trainer = trainer.ContrastphysTrainer.ContrastTrainer(config, data_loader_dict)
    elif config.MODEL.NAME == 'Greip':
        model_trainer = trainer.GreipTrainer.BaseNetTrainer(config, data_loader_dict)
    elif config.MODEL.NAME == 'QwenVL_rPPG':
        model_trainer = trainer.QwenVLrPPGTrainer.QwenVLrPPGTrainer(config, data_loader_dict)
    elif config.MODEL.NAME == 'QwenVL_Direct':
        model_trainer = trainer.QwenVLDirectTrainer.QwenVLDirectTrainer(config, data_loader_dict)
    else:
...
def test(config, data_loader_dict):
    ...
    if config.MODEL.NAME == "Physnet":
        ...
        model_trainer = trainer.NewPhysLLMTrainer_DG.NewPhysLLMTrainer(config, data_loader_dict)
    elif config.MODEL.NAME == 'RhythmFormer':
        model_trainer = trainer.RhythmFormerTrainer.RhythmFormerTrainer(config, data_loader_dict)
    elif config.MODEL.NAME == 'MDNet':
        model_trainer = trainer.MDNetTrainer.MDNetTrainer(config, data_loader_dict)  
...
if __name__ == "__main__":
    ...
    train_loader_dict = dict()  # 储存定义的类
    train_data_loader_dict = dict() # 储存定义的类的dataloader
    if config.TOOLBOX_MODE == "multi_train_and_test":
        for i in range(1, config.TRAIN.DATA.MULTI_SOURCE.NUM_SOURCE+1):
            # print(i)
            cur_loader = "train_loader" + str(i)
            if config.TRAIN.DATA.MULTI_SOURCE.SOURCE_NAME[i - 1] == "UBFC-rPPG":
                train_loader_dict[cur_loader] = data_loader.UBFCrPPGLoader.UBFCrPPGLoader
            elif config.TRAIN.DATA.MULTI_SOURCE.SOURCE_NAME[i - 1] == "PURE":
                train_loader_dict[cur_loader] = data_loader.PURELoader.PURELoader
            elif config.TRAIN.DATA.MULTI_SOURCE.SOURCE_NAME[i - 1] == "MMPD":
                train_loader_dict[cur_loader] = data_loader.MMPDLoader.MMPDLoader
            elif config.TRAIN.DATA.MULTI_SOURCE.SOURCE_NAME[i - 1] == "BUAA":
                train_loader_dict[cur_loader] = data_loader.BUAALoader.BUAALoader
            elif config.TRAIN.DATA.MULTI_SOURCE.SOURCE_NAME[i - 1] == "VitalVideo":
                train_loader_dict[cur_loader] = data_loader.VitalVideoLoader.VitalVideoLoader
            elif config.TRAIN.DATA.MULTI_SOURCE.SOURCE_NAME[i - 1] == "VIPL":
                train_loader_dict[cur_loader] = data_loader.VIPLHRLoader.VIPLHRLoader
            elif config.TRAIN.DATA.MULTI_SOURCE.SOURCE_NAME[i - 1] == "Zhuhai":
                train_loader_dict[cur_loader] = data_loader.ZhuhaiLoader.ZhuhaiLoader
            else:
                raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
                                             SCAMPS, BP4D+ (Normal and BigSmall preprocessing), UBFC-PHYS, iBVP and Zhuhai.")

        if config.TRAIN.DATA.DATASET and (
            config.TRAIN.DATA.DATA_PATH or config.TRAIN.DATA.MULTI_SOURCE.SOURCE_PATHS
        ):
            for i in range(1, config.TRAIN.DATA.MULTI_SOURCE.NUM_SOURCE+1):
                cur_loader1 = "train_data_loader" + str(i)
                cur_loader2 = "train_loader" + str(i)
                cur_data_loader = "train" + str(i)
                source_name = config.TRAIN.DATA.MULTI_SOURCE.SOURCE_NAME[i - 1]
                source_path = config.TRAIN.DATA.DATA_PATH
                if source_name in config.TRAIN.DATA.MULTI_SOURCE.SOURCE_PATHS:
                    source_path = config.TRAIN.DATA.MULTI_SOURCE.SOURCE_PATHS[source_name]
                train_data_loader_dict[cur_loader1] = train_loader_dict[cur_loader2](
                                                            name=cur_data_loader,
                                                            data_path=source_path,
                                                            config_data=config.TRAIN.DATA)
                data_loader_dict[cur_data_loader] = DataLoader(
                        dataset=train_data_loader_dict[cur_loader1],
                        num_workers=16,
        ...
        elif config.VALID.DATA.DATASET == "BUAA":
            valid_loader = data_loader.BUAALoader.BUAALoader
        elif config.TRAIN.DATA.DATASET == "COHFACE":
            valid_loader = data_loader.COHFACELoader.COHFACELoader
        elif config.TRAIN.DATA.DATASET == "VitalVideo":
            valid_loader = data_loader.VitalVideoLoader.VitalVideoLoader
        elif config.VALID.DATA.DATASET == "VIPL":
            valid_loader = data_loader.VIPLHRLoader.VIPLHRLoader
        elif config.VALID.DATA.DATASET == "Zhuhai":
            valid_loader = data_loader.ZhuhaiLoader.ZhuhaiLoader
    ...
    if config.TOOLBOX_MODE == "train_and_test":
        ...
        elif config.TRAIN.DATA.DATASET == "BUAA":
            train_loader = data_loader.BUAALoader.BUAALoader
        elif config.TRAIN.DATA.DATASET == "VitalVideo":
            train_loader = data_loader.VitalVideoLoader.VitalVideoLoader
        elif config.TRAIN.DATA.DATASET == "VIPL":
            train_loader = data_loader.VIPLHRLoader.VIPLHRLoader
        elif config.TRAIN.DATA.DATASET == "Zhuhai":
            train_loader = data_loader.ZhuhaiLoader.ZhuhaiLoader
        else:
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
                             SCAMPS, BP4D+ (Normal and BigSmall preprocessing), UBFC-PHYS, iBVP and Zhuhai.")
    ...
    if config.TOOLBOX_MODE == "train_and_test" or config.TOOLBOX_MODE == "only_test" or config.TOOLBOX_MODE == "multi_train_and_test":
        # test_loader
        ...
        elif config.TEST.DATA.DATASET == "BUAA":
            test_loader = data_loader.BUAALoader.BUAALoader
        elif config.TEST.DATA.DATASET == "VitalVideo":
            test_loader = data_loader.VitalVideoLoader.VitalVideoLoader
        elif config.TEST.DATA.DATASET == "VIPL":
            test_loader = data_loader.VIPLHRLoader.VIPLHRLoader
        elif config.TEST.DATA.DATASET == "Zhuhai":
            test_loader = data_loader.ZhuhaiLoader.ZhuhaiLoader
        else:
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
                             SCAMPS, BP4D+ (Normal and BigSmall preprocessing), UBFC-PHYS, iBVP and Zhuhai.")
        ...
        elif config.UNSUPERVISED.DATA.DATASET == "Zhuhai":
            unsupervised_loader = data_loader.ZhuhaiLoader.ZhuhaiLoader
        else:
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
                             SCAMPS, BP4D+, UBFC-PHYS, iBVP and Zhuhai.")
