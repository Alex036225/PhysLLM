...
from neural_methods.model.MLLMTrack.feature_fusion import MMFusion, MultiScaleFeatureFusion, MultiAverageFeatureFusion, FeatureProjection, OTFusion
...
from neural_methods.model.MLLMTrack.OT_utils import PotentialNet, AdvancedPotentialNet
# from neural_methods.model.OTTrack.transportnet import TransportNet_3D
# from neural_methods.model.OTTrack.wrapper import OTWrapper
from neural_methods.model.MLLMTrack.feature_fusion import Projector
...
class NewPhysLLM(nn.Module):
    def __init__(self, configs,):
        ...
        # #     feature_channels=feature_channels,  # 对应三个特征图的通道数[64, 64, 64], [32, 64, 64], [96, 96, 96]
        # #     token_dim=self.d_llm # 768
        # # )
        # self.projections = FeatureProjection(
        #     feature_channels=feature_channels,  # 对应三个特征图的通道数[64, 64, 64], [32, 64, 64], [96, 96, 96]
        #     token_dim=self.d_llm # 768
        # )
        ...
        # self.transportnet1 = TransportNet_3D(input_dim=self.d_llm , transport_dim=self.d_llm * 2)
        # self.transportnet2 = TransportNet_3D(input_dim=self.d_llm , transport_dim=self.d_llm * 2)
        # self.transportnet3 = TransportNet_3D(input_dim=self.d_llm , transport_dim=self.d_llm * 2)
        # self.multiprojector = Projector(feature_channels=feature_channels, token_dim=self.d_llm)

        # self.MMfuser = MMFuser(dim=self.d_llm)
    ...
    def forward(self, data):
        if self.configs.MODEL.VIDEO_ENC == "PhysNet":
            ...
            # x_fusion, features_list, phi_list = self.otfusion(
            #     features=[x_visual6464, x_visual3232, x_visual1616],
            #     target_len=self.target_sequence_length
            # )
            # print("features_list:", len(features_list))
            # print("phi_list:", len(phi_list))
            

            features_list = []
            # features_list.append(x_visual6464)
            # features_list.append(x_visual3232)
            # features_list.append(x_visual1616)

            # for x in features_list:
            #     # 计算均值和标准差 (按最后一维)
            #     mean = x.mean(dim=-1, keepdim=True)  # (4, 32, 1)
            #     std = x.std(dim=-1, keepdim=True)    # (4, 32, 1)

            #     # 标准化
            #     x = (x - mean) / (std + 1e-8)

            # features_list = self.multiprojector(features_list, 32)

            # x1 = self.transportnet1(features_list[0])
            # x2 = self.transportnet2(features_list[1])
            # x3 = self.transportnet3(features_list[2])
            features_ot = []
            # features_ot.append(x1)
            # features_ot.append(x2)
            # features_ot.append(x3)
            # for x in features_ot:
            #     # 计算均值和标准差 (按最后一维)
            #     mean = x.mean(dim=-1, keepdim=True)  # (4, 32, 1)
            #     std = x.std(dim=-1, keepdim=True)    # (4, 32, 1)
            ...
            # X = torch.cat(features_list[:-2], dim=1)
            # FL = features_list[-1]
            # x_fusion = self.MMfuser(X, FL)
        ...
        # x_fusion, phi = self.potentialNet(x_fusion)
        # 将ot对齐插入每层特征
        ...
        return modified_rppg, features_list, features_ot, x_fusion # [:, -self.pred_len:, :]
        # return modified_rppg , x_fusion # [:, -self.pred_len:, :]
