import os
from neural_methods.model.MLLMTrack.OT_utils import PotentialNet, ot_dual_loss_multi_domain, AdaptiveCost
import numpy as np
import torch
...
# from neural_methods.model.OTTrack.wrapper import MultiOTWrapper
from neural_methods.loss.coralloss import coral_loss_pairwise
from neural_methods.loss.MMDloss import pairwise_mmd_loss
from neural_methods.model.DOHA.HPS import self_similarity_calc
from neural_methods.model.DOHA.PCGrad import PCGrad
from neural_methods.loss.coralloss import coral_loss_pairwise
...
class NewPhysLLMTrainer(BaseTrainer):
    def __init__(self, config, data_loader):
        ...
        self.cost_fn = lambda x, y: torch.sum((x - y)**2, dim=-1).cuda()
        self.cost = AdaptiveCost().cuda()
        ...
        self.otwapper = MultiOTWrapper(input_dims=[1536*32, 1536*32, 1536*32]).cuda()
                ...
                with tqdm(total=max_len, ncols=100) as tbar:
                    for idx in range(max_len):
                        ...
                        for dataset_key in key_dict:
                            ...
                            # 前向传播
                            prediction, feature_list, feature_ot, x_fusion = self.model(facial_video)
                            # prediction, x_fusion = self.model(facial_video)
                            ot_feature.append(feature_ot)
                            raw_feature.append(feature_list)
                            feature_list.append(x_fusion)
                            ...
                            total_loss += loss1
                            # loss2_1 = self.otwapper.transport_loss(feature_ot[0], 0)
                            # total_loss += 0.1 * loss2_1
                            # loss2_2 = self.otwapper.transport_loss(feature_ot[1], 1)
                            # total_loss += 0.1 * loss2_2
                            # loss2_3 = self.otwapper.transport_loss(feature_ot[2], 2)
                            # total_loss += 0.1 * loss2_3
                            current_losses[dataset_key] = loss1.item()
                            epoch_losses[dataset_key].append(loss1.item())
                            running_losses[dataset_key] += loss1.item()
                        
                        # self.otwapper.setup_optimizer(lr=1e-4)
                        # ot_loss = self.otwapper.update_critics(raw_feature, ot_feature)
                        # ot_loss_val = self.otwapper.transport_loss(ot_feature)
                        # total_loss +=  ot_loss_val

                        # 反向传播和优化
                        # accelerator.backward(loss)
                        # feature_loss = pairwise_mmd_loss(feature_list)
                        # feature_loss = coral_loss_pairwise(feature_list)
                        # total_loss += feature_loss
                        total_loss.backward()
                        # 在 loss.backward() 之后执行
                        # torch.nn.utils.clip_grad_norm_(
                        #     self.model.parameters(),  # 对模型的所有参数进行裁剪
                        #     max_norm=2.0              # 最大梯度范数
                        # )
    ...
    def valid(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for idx, batch in enumerate(tbar):
                ...
                prediction, feature_fusion, _, _ = self.model(facial_video)
                # prediction, _ = self.model(facial_video, prompt)
                prediction = (prediction-torch.mean(prediction, axis=-1).view(-1, 1))/torch.std(prediction, axis=-1).view(-1, 1)    # normalize
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for idx, batch in enumerate(tbar):
                ...
                # 前向传播
                # prompt = self.con_encoder(facial_video_delete, strength, batch[2], batch[3], self.config.TEST.DATA.DATASET)
                prediction, _ , _, _= self.model(facial_video)
                prediction = (prediction-torch.mean(prediction, axis=-1).view(-1, 1))/torch.std(prediction, axis=-1).view(-1, 1)    # normalize
                label = label - torch.mean(label, dim=-1, keepdim=True)
                label = label / torch.std(label, dim=-1, keepdim=True)
        ...
        results = {'ground_truth': ground_truth_list, 'rppg': rppg_list}
        pred_exp_dir = os.path.join(
            self.config.TEST.OUTPUT_SAVE_DIR or os.path.join(self.model_dir, self.model_folder),
            "visual",
        )
        os.makedirs(pred_exp_dir, exist_ok=True)
        np.save(os.path.join(pred_exp_dir, 'bvp_UBFC_BL'), results)
    ...
    def _get_hr(self, signal, sr=30, min_hr=40, max_hr=180):
        """
    ...
    def _batch_get_hr(self, signals, sr=30, min_hr=40, max_hr=180):
        """
        Calculate heart rates for a batch of signals.
        Input:
            signals: A tensor of shape (batch_size, signal_length), e.g., (4, 128).
        Output:
            A tensor of shape (batch_size, 1), containing the heart rates for each signal.
        """
        # Ensure the input is a numpy array or convert it to one
        ...
        for i in range(signals.shape[0]):
            signal = signals[i]
            hr = self._get_hr(signal, sr=sr, min_hr=min_hr, max_hr=max_hr)
            heart_rates.append(hr)
        
        # Convert the list to a numpy array and reshape to (batch_size, 1)
        return np.array(heart_rates).reshape(-1, 1)
