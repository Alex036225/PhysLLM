...
def _detrend(input_signal, lambda_value):
    """Detrend PPG signal using sparse linear solve for better performance."""
    input_signal = np.asarray(input_signal, dtype=np.float64)
    signal_length = input_signal.shape[0]
    if signal_length < 3:
        return input_signal

    # Construct second-order difference matrix D with shape (n-2, n)
    diags_data = np.vstack([
        np.ones(signal_length - 2),
        -2 * np.ones(signal_length - 2),
        np.ones(signal_length - 2)
    ])
    D = sparse.diags(diags_data, [0, 1, 2], shape=(signal_length - 2, signal_length), format='csc')

    # Solve (I + lambda^2 D^T D) z = x, then output x - z
    I = sparse.eye(signal_length, format='csc')
    A = I + (lambda_value ** 2) * (D.T @ D)
    trend = splinalg.spsolve(A, input_signal)
    detrended_signal = input_signal - trend
...
def _compute_macc(pred_signal, gt_signal):
    ...
    pred = np.asarray(np.squeeze(pred), dtype=np.float64)
    gt = np.asarray(np.squeeze(gt), dtype=np.float64)

    min_len = int(np.min((len(pred), len(gt))))
    if min_len < 2:
        return 0.0
    ...
    gt_std = np.std(gt_centered)
    denom = pred_std * gt_std
    if denom < 1e-12:
        return 0.0

    fft_pred = np.fft.fft(pred_centered)
    fft_gt = np.fft.fft(gt_centered)
    circular_corr = np.fft.ifft(fft_pred * np.conjugate(fft_gt)).real

    circular_corr = circular_corr / (min_len * denom)
    macc = float(np.max(np.abs(circular_corr)))
    if not np.isfinite(macc):
        return 0.0
    return macc
...
def calculate_metric_per_video(predictions, labels, fs=30, diff_flag=True, use_bandpass=True, hr_method='FFT', need_macc=True):
    ...
    if use_bandpass:
        ...
        # TODO:2025.2.16
        # [b, a] = butter(1, [0.6 / fs * 2, 4 / fs * 2], btype='bandpass')
        predictions = scipy.signal.filtfilt(b, a, np.double(predictions))
        labels = scipy.signal.filtfilt(b, a, np.double(labels))
    
    macc = _compute_macc(predictions, labels) if need_macc else 0.0
